...
{
    public partial class TransportStaff : Form
    {
        ...
        {
            conn.Open();
            string query = "SELECT * FROM TransportStaff";
            SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(query, conn);
            SqlCommandBuilder sqlCommandBuilder = new SqlCommandBuilder(sqlDataAdapter);
            var dataSet = new DataSet();
            sqlDataAdapter.Fill(dataSet);
            DGVTransportStaff.DataSource = dataSet.Tables[0];
            conn.Close();
        }
        ...
        private void TransportStaff_Load(object sender, EventArgs e)
        {
