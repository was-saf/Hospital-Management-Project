...
namespace Hospital_Mangement_System
{
    public partial class OutGoingPatient : Form
    {
        ...
        {
            conn.Open();
            string query = "SELECT * FROM OutpatientPrescription";
            SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(query, conn);
            SqlCommandBuilder sqlCommandBuilder = new SqlCommandBuilder(sqlDataAdapter);
            var dataSet = new DataSet();
            sqlDataAdapter.Fill(dataSet);
            DGVOutgoingPatientList.DataSource = dataSet.Tables[0];
            conn.Close();
        }
        ...
        private void DGVOutgoingPatientList_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
