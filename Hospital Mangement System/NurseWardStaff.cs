...
namespace Hospital_Mangement_System
{
    public partial class NurseWardStaff : Form
    {
        ...
        {
            conn.Open();
            string query = "SELECT * FROM NurseWardStaff";
            SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(query, conn);
            SqlCommandBuilder sqlCommandBuilder = new SqlCommandBuilder(sqlDataAdapter);
            var dataSet = new DataSet();
            sqlDataAdapter.Fill(dataSet);
            DGVNurseList.DataSource = dataSet.Tables[0];
            conn.Close();
        }
        ...
        private void NurseWardStaff_Load(object sender, EventArgs e)
        {
